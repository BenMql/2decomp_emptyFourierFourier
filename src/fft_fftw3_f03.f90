...
module decomp_2d_fft
  ...
  integer, parameter, public :: IN_CORE_CHEBYSHEV = 4
...
contains
  ...
  subroutine fft_init_general(pencil, nx, ny, nz)
    ...
    if (format==PHYSICAL_IN_X) then  ! BM patch
       call decomp_info_init(nx/2+1, ny, nz, sp)
    else if ((format==PHYSICAL_IN_Z).or.(format==IN_CORE_CHEBYSHEV)) then !BM patch
  ...
  subroutine decomp_2d_fft_get_size(istart, iend, isize)
    ...
    if (format==PHYSICAL_IN_X) then 
       ...
       iend   = sp%zen
       isize  = sp%zsz
    else if ((format==PHYSICAL_IN_Z).or.(format==IN_CORE_CHEBYSHEV)) then
       istart = sp%xst
       iend   = sp%xen
       isize  = sp%xsz
    end if
  ...
  subroutine init_fft_engine
    ...
    else if (format == IN_CORE_CHEBYSHEV) then           ! BM patch
       
       ! For R2C/C2R tranforms                           ! BM patch
       call r2c_1m_z_plan(plan(0,3), ph, sp)             ! BM patch
       call c2c_1m_y_plan(plan(0,2), sp, FFTW_FORWARD )  ! BM patch
       call c2c_1m_y_plan(plan(2,2), sp, FFTW_BACKWARD)  ! BM patch
       call c2r_1m_z_plan(plan(2,3), sp, ph)             ! BM patch
  ...
  subroutine fft_3d_r2c(in_r, out_c)
    ...
    ! ############################################### ! BM patch
                                                      ...
                                                      ! BM patch
       ! ===== 1D FFTs in Z =====                     ! BM patch
       call r2c_1m_z(in_r,wk13)                       ! BM patch
                                                      ! BM patch
       ! ===== Swap Z --> Y; 1D FFTs in Y =====       ! BM patch
       if (dims(1)>1) then                            ! BM patch
          call transpose_z_to_y(wk13,wk2_r2c,sp)      ! BM patch
          call c2c_1m_y(wk2_r2c,-1,plan(0,2))         ! BM patch
       else  ! out_c==wk2_r2c if 1D decomposition     ! BM patch
          call transpose_z_to_y(wk13,out_c,sp)        ! BM patch
          call c2c_1m_y(out_c,-1,plan(0,2))           ! BM patch
       end if                                         ! BM patch
                                                      ! BM patch
       ! ===== Swap Y --> X; 1D FFTs in X =====       ! BM patch
       if (dims(1)>1) then                            ! BM patch
          call transpose_y_to_x(wk2_r2c,out_c,sp)     ! BM patch
       end if                                         ! BM patch
       ! the dct transform takes place in the         ! BM patch
       ! calling routine.                             ! BM patch
    ! ############################################### ! BM patch
  ...
  subroutine fft_3d_c2r(in_c, out_r)
    ...
    ! ################################################### ! BM patch
                                                          ...
                                                          ! BM patch
       ! ===== 1D FFTs in X =====                         ! BM patch
#ifdef OVERWRITE                          
       ! the dct transform takes place in the             ! BM patch
       ! calling routine.                                 ! BM patch
#else                                     
       sz = sp%xsz(1)*sp%xsz(2)*sp%xsz(3)                 ! BM patch
       wk1_p = fftw_alloc_complex(sz)                     ! BM patch
       call c_f_pointer(wk1_p, wk1, &
                   [sp%xsz(1),sp%xsz(2),sp%xsz(3)])       ! BM patch
       wk1 = in_c                                         ! BM patch
       ! the dct transform takes place in the             ! BM patch
       ! calling routine.                                 ! BM patch
#endif                                     
                                                          ! BM patch
       ! ===== Swap X --> Y; 1D FFTs in Y =====           ! BM patch
       if (dims(1)>1) then                                ! BM patch
#ifdef OVERWRITE                                 
          call transpose_x_to_y(in_c,wk2_r2c,sp)          ! BM patch
#else                                       
          call transpose_x_to_y(wk1,wk2_r2c,sp)           ! BM patch
#endif                                       
          call c2c_1m_y(wk2_r2c,1,plan(2,2))              ! BM patch
       else  ! in_c==wk2_r2c if 1D decomposition          ! BM patch
#ifdef OVERWRITE                                 
          call c2c_1m_y(in_c,1,plan(2,2))                 ! BM patch
#else                                         
          call c2c_1m_y(wk1,1,plan(2,2))                  ! BM patch
#endif                                       
       end if                                             ! BM patch
                                                          ! BM patch
       ! ===== Swap Y --> Z; 1D FFTs in Z =====           ! BM patch
       if (dims(1)>1) then                                ! BM patch
          call transpose_y_to_z(wk2_r2c,wk13,sp)          ! BM patch
       else                                               ! BM patch
#ifdef OVERWRITE                                  
          call transpose_y_to_z(in_c,wk13,sp)             ! BM patch
#else                                        
          call transpose_y_to_z(wk1,wk13,sp)              ! BM patch
#endif                                      
       end if                                             ! BM patch
       call c2r_1m_z(wk13,out_r)                          ! BM patch
    ! ################################################### ! BM patch
